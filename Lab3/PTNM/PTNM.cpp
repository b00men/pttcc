...
void check_properties(packet *pkt)
{
	...
	if(pkt->transport_type == UDP)
	{
		...
		cout << "Property violation! Packet " << pkt->location_in_trace << " is UDP!, source port:" << sport <<" destination port:" << dport <<"\n";
	}
}
...
void process_packet(u_char *args, const struct pcap_pkthdr *pkthdr, const u_char *pkt)
{
	...
	//The previousl line can be used for debug... it is priceless
